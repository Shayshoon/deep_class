...
def torch_temporary_seed(seed: int):
    ...
    try:
        # ====== YOUR CODE: ======
        new_state = torch.random.manual_seed(seed)
        # ========================
        yield
    finally:
        # ====== YOUR CODE: ======
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index < 0 or index >= self.subset_len:
            raise IndexError("Index out of bounds for the subset.")

        absolute_index = index + self.offset
        
        return self.source_dataset[absolute_index]
        # ========================
